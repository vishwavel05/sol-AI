# SOL AI — Browser Extension (Manifest V3)

The **SOL AI Browser Extension** allows users to encounter Tamil text on any webpage, select any Tamil word or phrase, and receive contextual etymological, morphological, dictionary, and Sangam literary evidence analysis directly inside a floating side panel.

---

## 🌟 Key Features

1. **Right-Click Context Menu**: Select any Tamil text on any webpage, right-click, and choose **"Explain with SOL AI"**.
...
> All LLM credentials and resource databases remain strictly server-side on the SOL AI Python backend (`backend/api/server.py`). The extension communicates solely with the SOL AI API server.
...
Background Service Worker (service-worker.js)
    ↕ HTTP POST /api/query
SOL AI API Server (localhost:8000)
...
### Step 1: Start SOL AI API Server
Ensure the SOL AI Python backend is running locally:
```bash
...
3. Right-click and choose **"Explain with SOL AI"**.
...
1. Click the **SOL AI** extension icon in your toolbar.
...
- `contextMenus`: Creates "Explain with SOL AI" right-click menu item.
...
- **Panel shows "SOL AI could not be reached"**: Ensure `python backend/api/server.py --port 8000` is running locally and `http://localhost:8000/api/health` returns status `ok`.
- **Extension fails to reload changes**: Go to `chrome://extensions` and click the reload icon on the SOL AI extension card.
