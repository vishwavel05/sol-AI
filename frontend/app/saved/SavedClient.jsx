...
export default function SavedClient() {
  ...
  const clearSaved = () => {
    localStorage.removeItem("sol_saved_words");
    setSavedWords([]);
    setSelectedWord(null);
  };

  const removeWord = (query, e) => {
    e.preventDefault();
    e.stopPropagation();
    const updated = savedWords.filter(item => item.query !== query);
    localStorage.setItem("sol_saved_words", JSON.stringify(updated));
    setSavedWords(updated);
    if (selectedWord && selectedWord.query === query) {
      setSelectedWord(null);
    }
  };
            ...
            <div className="mx-auto w-full max-w-[1200px] px-4 sm:px-6 lg:px-8 xl:px-10 pt-6">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 border-b border-[var(--sol-line)] pb-6">
                <div className="flex items-center gap-3">
                  <div className="p-3 bg-[#111] rounded-2xl border border-[var(--sol-line)] text-red-500">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[var(--sol-muted)] font-serif-english italic mt-0.5">
                      Your personal collection of Tamil words
                    </p>
                  </div>
                </div>
              ...
              {!mounted ? (
                ...
                <div className="text-center py-20 flex flex-col items-center">
                  <div className="w-20 h-20 bg-[#111] border border-[var(--sol-line)] rounded-full flex items-center justify-center mb-4 text-slate-500 opacity-50">
                    <Heart className="w-8 h-8" />
                  </div>
                  <h2 className="text-xl font-bold text-[var(--sol-cream)] mb-2 font-serif-tamil">No saved words</h2>
                  <p className="text-[var(--sol-muted)] mb-6 max-w-sm">
                    You haven't saved any words yet. Click the Save button when exploring a word to add it here.
                  </p>
                  <Link
                    href="/"
                  ...
                  {savedWords.map((item, idx) => (
                    ...
                    >
                      <div className="flex justify-between items-start mb-4 w-full">
                        ...
                        <div
                          onClick={(e) => removeWord(item.query, e)}
                          className="w-8 h-8 rounded-full bg-[#111] flex items-center justify-center text-red-500 hover:bg-red-500 hover:text-white transition-colors border border-red-500/30 shrink-0 ml-4 cursor-pointer"
                      ...
                      <div className="mt-5 pt-3 border-t border-[var(--sol-line)]/50 text-xs text-[var(--sol-muted)] flex justify-between items-center w-full">
                        {item.query !== item.lemma && item.lemma ? (
                          ...
                          <span className="flex items-center gap-1 group-hover:text-[var(--sol-gold-bright)] transition-colors">
